...
const drawCloud: DeploySceneDraw = (ctx, w, h) => {
  ...
  ctx.strokeRect(w * 0.08, h * 0.16, w * 0.84, h * 0.68);
  ctx.setLineDash([]);

  // Instances inside the boundary — a small grid of squares.
  const cols = 3;
  const rows = 2;
  const bw = w * 0.17;
  const bh = h * 0.17;
  for (let r = 0; r < rows; r += 1) {
    for (let c = 0; c < cols; c += 1) {
      ink(ctx, (r + c) % 2 === 0 ? 0.95 : 0.55);
      ctx.fillRect(w * 0.18 + c * (bw + w * 0.055), h * 0.29 + r * (bh + h * 0.12), bw, bh);
    }
  }
};
...
const drawManaged: DeploySceneDraw = (ctx, w, h) => {
  // Workload block.
  ink(ctx, 0.95);
  ctx.fillRect(w * 0.24, h * 0.16, w * 0.52, h * 0.34);
  ctx.save();
  ctx.globalCompositeOperation = "destination-out";
  for (let u = 0; u < 3; u += 1) {
    ctx.fillStyle = "rgba(0,0,0,0.85)";
    ctx.fillRect(w * 0.3, h * 0.22 + u * h * 0.1, w * 0.4, h * 0.035);
  }
  ctx.restore();

  // The hands under it: two supporting brackets rising to meet the block.
  ink(ctx, 0.6);
  ctx.lineWidth = Math.max(2, w * 0.022);
  ctx.beginPath();
  ctx.moveTo(w * 0.14, h * 0.86);
  ctx.quadraticCurveTo(w * 0.16, h * 0.58, w * 0.4, h * 0.55);
  ctx.stroke();
  ctx.beginPath();
  ctx.moveTo(w * 0.86, h * 0.86);
  ctx.quadraticCurveTo(w * 0.84, h * 0.58, w * 0.6, h * 0.55);
  ctx.stroke();

  // Ubik's mark sitting at the base — the operator on duty.
  ink(ctx, 1);
  ctx.fillRect(w * 0.44, h * 0.76, w * 0.12, h * 0.12);
};
